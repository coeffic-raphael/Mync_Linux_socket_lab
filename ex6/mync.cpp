...
//create server Unix Domain Socket Datagram
int create_uds_server_datagram(const char* path) {
    ...
    unlink(path);  // delete the file if exist
...
//create client Unix Domain Socket Datagram
int create_uds_client_datagram(const char* path) {
    int sockfd = socket(AF_UNIX, SOCK_DGRAM, 0);
    if (sockfd < 0) {
        std::cerr << "Error opening socket\n";
        return -1;
...
// create server Unix Domain Socket Stream
int create_uds_server_stream(const char* path) {
...
// create client Unix Domain Socket Stream
int create_uds_client_stream(const char* path) {
    int sockfd = socket(AF_UNIX, SOCK_STREAM, 0);
    if (sockfd < 0) {
        std::cerr << "Error opening socket\n";
        return -1;
...
int main(int argc, char *argv[]) {
    ...
    bool should_close = false; // for determine if we should close the input/output file descriptors
    int timeout = 0;
    struct sockaddr_in serv_addr;
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-e") == 0 && i + 1 < argc) {
            ...
            if (strncmp(argv[i], "TCPS", 4) == 0) {
                ...
                server_fd = create_uds_server_datagram(path);
                if (server_fd < 0) return 1;
                input_fd = server_fd;
                should_close = true;
            } else if (strncmp(argv[i], "UDSSS", 5) == 0) {
                const char* path = argv[i] + 5;
                server_fd = create_uds_server_stream(path);
                if (server_fd < 0) return 1;
                client_fd = accept(server_fd, NULL, NULL);
                if (client_fd < 0) {
                    std::cerr << "Error accepting connection\n";
            ...
            if (strncmp(argv[i], "TCPC", 4) == 0) {
                ...
                const char* path = argv[i] + 5;
                client_fd = create_uds_client_datagram(path);
                if (client_fd < 0) return 1;
                output_fd = client_fd;
                should_close = true;
            } else if (strncmp(argv[i], "UDSCS", 5) == 0) {
                const char* path = argv[i] + 5;
                client_fd = create_uds_client_stream(path);
                if (client_fd < 0) return 1;
                output_fd = client_fd;
                should_close = true;
            }
    ...
    if (command) {
        ...
        if (pid == -1) {
            ...
            exit(1); //check that child finish if exelp fail
        } else {
            // parent
            int status;
            waitpid(pid, &status, 0); // wait the child
            if (should_close) {

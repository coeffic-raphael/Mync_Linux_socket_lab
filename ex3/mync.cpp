...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " -e \"command\" [-i/-o/-b [TCPS<PORT>|TCPC<IP,PORT>]]\n";
        return 1;
    }
        ...
        if (strcmp(argv[i], "-e") == 0) {
            ...
            if (i + 1 < argc) {
                ...
                if (strncmp(argv[i], "TCPS", 4) == 0) {
                    ...
                    should_close = true; //we should close these sockets when done
                } else if (strncmp(argv[i], "TCPC", 4) == 0) {
                    char *colon = strchr(argv[i] + 4, ',');
                    if (colon) {
                        *colon = '\0'; //terminate the string to isolate the IP
